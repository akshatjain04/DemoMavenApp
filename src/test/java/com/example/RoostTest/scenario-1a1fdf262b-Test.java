...
// Add additional dependencies and libraries here

public class CreatePetWithBodyGivenTheApiRunningAtHttpPetstoreSwaggerIoV2WhenAPostRequestToPetIsMadeAndTheRequestBodyIsNameDoggieStatusAvailableThenA200ResponseIsReturnedWithin50MsAndTheResponseBodyContainsId  { 
  
  private static String API_HOST;
  private static RequestSpecification requestSpec;
  
  // The @BeforeAll annotation is to execute the method "setUp" before running any tests.
  @BeforeAll
  public static void setUp() {    
    API_HOST = System.getenv("API_HOST"); // Will fetch the base URL from env variable
  ...
  public void create_pet() throws Exception {
    String scenarioFileName = "scenario-1a1fdf262b-Test.csv";	
    List<String> lines = Files.readAllLines(Paths.get(scenarioFileName)); 
    for (int i = 1; i < lines.size(); i++) {  // Start from the second line, as first one contains headers
      String[] columns = lines.get(i).split("\\^\\|\\^");
      Map<?, ?> requestBody = new Gson().fromJson(columns[3], Map.class);
      Map<String, String> requestHeaders = new Gson().fromJson(columns[2], Map.class);
      Map<?, ?> expectedResponseBody = new Gson().fromJson(columns[5], Map.class);
      int expectedResponseCode = Integer.parseInt(columns[4]);
      ...
      given()
        ...
        .body(requestBody)
      .when()
        .request(columns[0], columns[1])  // columns[0]: Method, columns[1]: URL
      .then()
        .statusCode(expectedResponseCode)
        .extract()
        .response();
     
      assertEquals(expectedResponseBody, response.jsonPath().getMap("$"));
   }
  }

  @AfterAll
  public static void tearDown() {
    requestSpec = null;  // Free up resources after all tests are completed
  }
}
