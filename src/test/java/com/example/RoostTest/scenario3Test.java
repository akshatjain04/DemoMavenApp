...
public class scenario3Test {
    ...
    public void createPet() throws Exception {
        ...
        BufferedReader br = new BufferedReader(new FileReader("src/test/java/com/example/RoostTest/scenario3Test.csv"));
        String line;
        ...
        while ((line = br.readLine()) != null) {
            ...
            String requestPayload = (values[3]);
            int expectedResponseCode = Integer.parseInt(values[4]);
            JSONObject expectedResponsePayload = new JSONObject(values[5]);

            if (method.equals("POST")) {
                Response response = given().baseUri(API_HOST).headers(headers)
                        .body(requestPayload).when()
                        .post(url).then().extract().response();
                // incorrect key value pairs
                // for (Map.Entry<String, String> entry : expectedResponsePayload.entrySet()) {
                // response.then().body(entry.getKey(), equalTo(entry.getValue().toString()));
                // }
                response.then().statusCode(expectedResponseCode);
                assertEquals(response.jsonPath().get("category.name"),
                        expectedResponsePayload.getJSONObject("category").getString("name"));
                assertEquals(response.jsonPath().get("status"),
                        expectedResponsePayload.getString("status"));
            }
