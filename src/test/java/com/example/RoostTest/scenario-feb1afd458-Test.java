...
class ScenarioTest {
    ...
    @BeforeAll
    public static void initialiseUrl() {
        BASE_URL = System.getenv().get("API_HOST");
        if (BASE_URL == null) {
            throw new IllegalStateException("Environment variable API_HOST not found!");
        }
    }
    ...
    public void createPetTest() throws Exception {
        // Initialize path to the csv data file for this particular test case
        String csvFilePath = "src/test/java/com/example/RoostTest/scenario-feb1afd458-Test.csv";

        try (Reader reader = new FileReader(csvFilePath.replace("/", FileSystems.getDefault().getSeparator()))) {

            // Initialize CSV to Bean converter
            ColumnPositionMappingStrategy<TestDataModel> strategy = new ColumnPositionMappingStrategy<>();
            strategy.setType(TestDataModel.class);

            CsvToBean<TestDataModel> csvToBean = new CsvToBeanBuilder(reader)
            ...
            // Fetch the test data rows from the csv file
            List<TestDataModel> dataModels = csvToBean.parse();

            for (TestDataModel dataModel : dataModels) {

                // Fetch the headers from each row
                Map<String, String> headers = new HashMap<>();
                JSONObject headerObject = (JSONObject)new JSONParser().parse(dataModel.getReqHeaders());
                for (Object key : headerObject.keySet()) {
                    headers.put((String) key, (String) headerObject.get(key));
                }

                // Fetch the API request body from each row
                String apiRequestBody = dataModel.getReqBody();

                // Make a POST request to create a pet
                Response response = given().baseUri(BASE_URL)
                        .headers(headers)
                        .contentType(ContentType.JSON)
                        .body(apiRequestBody)
                        .log().all()
                        .when().post(dataModel.getUrl())
                        .then()
                        .statusCode(Integer.parseInt(dataModel.getResponseCode()))
                        .log().all().extract().response();

                // Assert that the response body has the expected id
                JSONObject expectedResponseObject = (JSONObject)new JSONParser().parse(dataModel.getResponseBody());
                for (Object key : expectedResponseObject.keySet()) {
                    assertEqual(response.path((String) key), expectedResponseObject.get(key));
                }
            }
        }
    }
